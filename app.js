...
import token from './src/token.did'
import bridge from './src/dfinity-bridge.did'
...
async function onBtnConnect() {
  try {
    ...
    const res4445 = await window.ic.plug.getTransactions();
    console.log(res4445)
   ...
   // outputWrite(`approve() call response ${res3}`);
    //const res4 = await BridgeActor.requestBridgingToEnd(BigInt(100000000),Principal.fromText('oa67n-laaaa-aaaai-qfm3q-cai'))
    //console.log(res4)
    //outputWrite(`approve() call response ${res4}`);
  }catch (e) {
    console.log(e)
  }
}
